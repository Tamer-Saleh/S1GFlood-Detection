...
## Preamble
```yaml
The following animation shows SAR images before and after the event and a sample of flooded area findings 
for a rural area in Iran that was hit by a flood in March 2019. The right figure provides an enlarged visual 
of a 1 km x 1 km area within the larger area as in the yellow box, showing the size of the affected buildings 
from the flood by our model.
```
